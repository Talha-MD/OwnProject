  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:9000/api/products?keyword=${keyword}`
      );
...
export const deleteProduct = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`http://localhost:9000/api/products/${id}`, config);
    dispatch({
...
export const createProductReview =
  (productId, review) => async (dispatch, getState) => {
    try {
      ...
      await axios.post(
        `http://localhost:9000/api/products/${productId}/reviews`,
        review,
        config
      );
